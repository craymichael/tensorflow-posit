...
REGISTER_OP("AvgPool")
    ...
    .Attr("T: {half, bfloat16, float, double, posit16}")
...
REGISTER_OP("FusedBatchNormV2")
    ...
    .Attr("T: {half, bfloat16, float, posit16}")
    .Attr("U: {float}")
    .Attr("epsilon: float = 0.0001")
    .Attr("data_format: string = 'NHWC'")
    .Attr("is_training: bool = true")
...
REGISTER_OP("MaxPool3DGrad")
    ...
    .Attr(GetPaddingAttrString())
    .Attr(GetConvnet3dDataFormatAttrString())
    .Attr("T: {half, bfloat16, float, posit16} = DT_FLOAT")
    .Attr("TInput: {half, bfloat16, float, posit16} = DT_FLOAT")
    .SetShapeFn([](InferenceContext* c) {
      return UnchangedShapeWithRank(c, 5);
    });
