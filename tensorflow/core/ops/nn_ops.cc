...
REGISTER_OP("AvgPool")
    ...
    .Attr("T: {half, bfloat16, float, double, posit8, posit16, posit32}")
...
REGISTER_OP("FusedBatchNormV2")
    ...
    .Attr("T: {half, bfloat16, float, posit8, posit16, posit32}")
    .Attr("U: {float}")
    .Attr("epsilon: float = 0.0001")
    .Attr("data_format: string = 'NHWC'")
    .Attr("is_training: bool = true")
...
REGISTER_OP("FusedBatchNormGradV2")
    ...
    .Attr("T: {half, bfloat16, float, posit8, posit16, posit32}")
    .Attr("U: {float, posit8, posit16, posit32}")
    .Attr("epsilon: float = 0.0001")
    .Attr("data_format: string = 'NHWC'")
    .Attr("is_training: bool = true")
    .SetShapeFn(shape_inference::FusedBatchNormGradShape);
...
REGISTER_OP("MaxPool3DGrad")
    ...
    .Attr(GetPaddingAttrString())
    .Attr(GetConvnet3dDataFormatAttrString())
    .Attr("T: {half, bfloat16, float, posit8, posit16, posit32} = DT_FLOAT")
    .Attr("TInput: {half, bfloat16, float, "
          "posit8, posit16, posit32} = DT_FLOAT")
    .SetShapeFn([](InferenceContext* c) {
      return UnchangedShapeWithRank(c, 5);
    });
...
REGISTER_OP("MaxPool")
    .Attr(
        ...
        "uint16, qint8, posit8, posit16, posit32} = DT_FLOAT")
    .Attr("ksize: list(int) >= 4")
    .Attr("strides: list(int) >= 4")
    .Attr(GetPaddingAttrString())
